...
similarPopups.forEach((offer) => {
  // Функция вызывается ради побочного эффекта
  ...
  popupElement.querySelector(
    '.popup__text--price'
  ).textContent = `${offer.offer.price} Р/ночь`;
  popupElement.querySelector('.popup__type').textContent = offer.offer.type;
  popupElement.querySelector(
    '.popup__text--capacity'
  ).textContent = `${offer.offer.rows} комнат для ${offer.offer.quest} гостей`;
  popupElement.querySelector(
    '.popup__text--time'
  ).textContent = `Заезд после ${offer.offer.checkin}, выезд до ${offer.offer.checkout}`;
  popupElement.querySelector('.popup__features').children.textContent =
